...
public class FightGame {
    public void gameStart(User user) {
        ...
        skills.add(new Skill("普通攻击",  ""));
        skills.add(new Skill("强力一击", "(20MP)"));
        skills.add(new Skill("生命汲取",  "(20MP 恢复30%血量)"));
        skills.add(new Skill("蓝量回复", "(恢复50%蓝量)"));
    ...
    public void playerRound(HeroCharacter player, EnemyCharacter enemy){
        ...
        for (int i = 0; i < player.skills.length; i++) {
            ...
            System.out.println(i + 1 + ". " + player.skills[i].getName() + player.skills[i].getDescription());
        }
        Scanner sc = new Scanner(System.in);
        ...
        switch (choice){
            case 1 -> {
                ...
                System.out.println(player.getName()+" 对 " + enemy.getName() + " 使用了 普通攻击 ,造成 " + enemy.takeDamage(damage1) + " 点伤害");
            }
            case 2 -> {
                if(player.getMP() >= 20){
                    player.setMP(player.getMP() - 20);
                    int damage2 = calculateDamage((int) (player.getAttack() * 1.8), enemy.getDefence());
                    System.out.println(player.getName()+" 对 " + enemy.getName() + " 使用了 强力一击 ,造成 " + enemy.takeDamage(damage2) + " 点伤害");
                }else {
            ...
            case 3 -> {
                if(player.getMP() >= 20){
                    ...
                    System.out.println(player.getName()+" 对 " + enemy.getName() + " 使用了 生命汲取 ,造成了 "+ enemy.takeDamage(damage3) +" 点伤害,恢复了 " + heal + " 点生命值");
                    player.heal(heal);
                }else {
                    System.out.println("当前法力值不足,自动释放普通攻击");
                    int damage1 = calculateDamage((int) (player.getAttack() * 1.5), enemy.getDefence());
            ...
            case 4 -> {
                if(player.isMPHealing()){
                    System.out.println("本回合已使用过该技能,自动释放普通攻击");
                    int damage1 = calculateDamage((int) (player.getAttack() * 1.5), enemy.getDefence());
                    System.out.println(player.getName()+" 对 " + enemy.getName() + " 使用了 普通攻击 ,造成 " + enemy.takeDamage(damage1) + " 点伤害");
                }else{
                    player.setMPHealing(true);
                    int healMP = (int) (player.getMaxMP() * 0.5);
    ...
    private void enemyTurn(EnemyCharacter enemy, HeroCharacter player) {
        ...
        switch (action) {
            case "普通攻击"->{
                ...
                System.out.println(enemy.getName()+" 对 " + player.getName() + " 使用了 普通攻击 ,造成 " + player.takeDamage(damage1) + " 点伤害");
            }
            case "猛击"->{
                if(enemy.getMP() >= 20){
                    enemy.setMP(enemy.getMP() - 20);
                    int damage2 = calculateDamage((int) (enemy.getAttack() * 1.5), player.getDefence());
                    System.out.println(enemy.getName() + " 对 " + player.getName() + " 使用了 猛击 ,造成 " + player.takeDamage(damage2) + " 点伤害");
                }else {
            ...
            case "快速攻击"->{
                if (enemy.getMP() >= 20) {
                    ...
                    System.out.println(enemy.getName()+" 对 " + player.getName() + " 使用了 快速攻击 ,造成 2 次" + player.takeDamage(damage3) / 2 + " 点伤害");
                }else {
                    System.out.println(enemy.getName() + "使用了 蓝量回复");
                    enemy.healMP(20);
                }
            ...
            case "天动万象"->{
                if (enemy.getMP() >= 50) {
                    ...
                    System.out.println(enemy.getName()+" 对 " + player.getName() + " 使用了 天动万象 ,造成 " + player.takeDamage(damage) + " 点伤害,恢复了 "+ heal +"点生命值");
                    enemy.heal(heal);
                }else {
                    System.out.println(enemy.getName() + "使用了 蓝量回复");
                    enemy.healMP(25);
