...
public class FightGame {
    ...
    public void enemyTurn(EnemyCharacter enemy, HeroCharacter player, int round,String plannedAction) throws InterruptedException {
        ...
        switch (plannedAction) {
            ...
            case "唯我独尊"->{
                //你马哥 ：增加技能：唯我独尊：提升10%的攻击力和防御力，降低player10%的攻击力和防御力，每场战斗限一次,消耗50mp
                enemy.setLastAddAttackRound(round);
                enemy.setMP(enemy.getMP() - 50);
                enemy.setAttack(enemy.getAttack() + (int) (enemy.getAttack() * 0.1));
                enemy.setDefence(enemy.getDefence() + (int) (enemy.getDefence() * 0.1));
                player.setAttack(player.getAttack() - (int) (player.getAttack() * 0.1));
                player.setDefence(player.getDefence() - (int) (player.getDefence() * 0.1));
                System.out.print(" 天 ");
                Thread.sleep(300);
                System.out.print(" 上 ");
                Thread.sleep(300);
